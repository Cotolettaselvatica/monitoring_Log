-- Estensione schema per il cruscotto (stesso DB degli aggregator).
-- La tabella conteggi_pezzi e' creata da UNIX_log_aggregator / WIN_log_aggregator.
...
-- Permessi per l'utente applicativo (eseguire come postgres se necessario):
-- GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO counter;
-- GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA public TO counter;
